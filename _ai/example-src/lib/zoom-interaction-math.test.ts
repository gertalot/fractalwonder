// ABOUTME: Test suite that EXPOSES bugs in zoom-to-point interaction at extreme zoom levels
// ABOUTME: These tests MUST FAIL to demonstrate the broken behavior at zoom 10^27+
...
function calculateScale(canvasHeight: number, zoom: Decimal): Decimal {
  ...
  try {
    return new Decimal(INITIAL_FRACTAL_VIEW_HEIGHT)
      .div(new Decimal(canvasHeight))
      .div(zoom);
  } finally {
...
function verifyPointerInvariant(
  ...
  const fractalBefore = pixelToFractalCoordinateUltraHP(
    pointerPixel,
    canvasWidth,
    canvasHeight,
    oldCenter,
  ...
  const fractalAfter = pixelToFractalCoordinateUltraHP(
    pointerPixel,
    canvasWidth,
    canvasHeight,
    newCenter,
...
function calculateRoundTripError(
  ...
  const roundTrip = fractalToPixelCoordinateUltraHP(
    fractal,
    canvasWidth,
    canvasHeight,
    center,
...
describe("Zoom Interaction Mathematics - EXPOSING BUGS AT EXTREME ZOOM", () => {
  describe("CRITICAL BUG: Real Scenario from Big Boss at zoom 5.5e27", () => {
    it("MUST FAIL: Pointer invariant is violated at extreme zoom (zoom 5.5e27 -> 5.5e28)", () => {
      ...
      const oldCenter = {
        x: new Decimal("0."),
        y: new Decimal("-0."),
      };
...
describe("Zoom Interaction Mathematics - Extreme Precision", () => {
  ...
  describe("Test 5: Round-trip Pixel→Fractal→Pixel Accuracy", () => {
    it("should maintain sub-pixel accuracy at zoom 10^27", () => {
      ...
      const error = calculateRoundTripError(
        testPixel,
        center,
        zoom,
        canvasWidth,
