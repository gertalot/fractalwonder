...
use fractalwonder_gpu::{
    GpuAvailability, GpuContext, GpuPerturbationHDRRenderer, ProgressiveGpuRenderer,
};
use leptos::*;
...
impl ParallelRenderer {
    ...
    ///
    /// NOTE: Currently unused - replaced by start_progressive_gpu_render with row-sets.
    #[allow(dead_code)]
    fn start_gpu_render(&self, viewport: &Viewport, canvas: &HtmlCanvasElement, tile_size: u32) {
        let width = canvas.width();
        let height = canvas.height();
...
///
/// NOTE: Currently unused - replaced by schedule_row_set for progressive rendering.
#[allow(dead_code)]
